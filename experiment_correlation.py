...
1. На исходных текстах генерируются гипотезы разными провайдерами (или одной моделью с разными параметрами)
2. Для каждой гипотезы считается COMET по триплету (исходник, гипотеза, эталон)
3. Для каждой гипотезы считается агрегированная безэталонная оценка
4. Проверяется корреляция Спирмена/Пирсона между двумя рядами оценок

Результат сохраняется в Excel файл для последующего анализа и построения графиков.
"""
...
def run_experiment(args):
    ...
    # Создаем конфигурации для генерации (температуры для вариативности)
    temperatures = args.temperatures if args.temperatures else [0.3, 0.5, 0.7, 0.9, 1.2]
    ...
    print(f"\nСохранение результатов в {output_excel}...")
...
def parse_args():
    ...
    parser.add_argument("--temperatures", type=float, nargs='+',
                        default=None,
                        help="Список температур для генерации разброса качества (например: 0.3 0.5 0.7 0.9 1.2)")
